...
shell-setup:
	...
	docker run --privileged -d --name port_shell -p :22 --volumes-from data -v /tmp/work:/tmp/work -v /var/run/docker.sock:/var/run/docker.sock -ti port_shell /usr/sbin/sshd -D -o UseDNS=no -o UsePAM=yes -o PasswordAuthentication=no -o UsePrivilegeSeparation=sandbox
